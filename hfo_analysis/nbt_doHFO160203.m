...
% *** 
% *** addoptation for Stellate visual markings
% ***
% ***----------------------------------------------------------------------
% *** Analysis:
...
% ***
% *** load('example1.mat')
% *** [HFOobj, results] = nbt_doHFO141226(data, 2000, 80, 500, channel_name);
% ***
% *** ---------------------------------------------------------------------
% *** for Matlab R12
...
% *** 
% =========================================================================
function [HFOobj, results] = nbt_doHFO160203(sig, Signal_filtered, hp, mark, p, input)
%NBT interface
...
if  strcmp(mark, 'Ripple')
    HFOobj.minNumberOscillations = 6;  % varies for Ripple and FR ?
elseif strcmp(mark, 'FastRipple')
    HFOobj.minNumberOscillations = 6;  % varies for Ripple and FR ?
end
...
% rmsWindow=10;
% env(1:rmsWindow)=0;
% for i=rmsWindow:length(Signal_filtered)
%     env(i)=sqrt( (1/rmsWindow) *(sum (Signal_filtered(i-rmsWindow+1:i).^2) ) );
% end

% ---------------------------------------------------------------------
% 3.
% threshold
% HFOobj.THR = 3 * std(env) + mean(env); %old threshold,

% if strcmp(mark, 'Ripple') % smooth for high frequency noise if ripple
%     Signal_filtered = smooth(Signal_filtered, round(1*1/(HFOobj.hpHigh+50)*HFOobj.fs));
% end
...
%             set(gca,'YDir','normal');
    ...
    display(['For ' num2str(floor(length(sigfull)/HFOobj.fs)) ' sec, baseline length = ' num2str(length(indHighEntr)) ])
end
