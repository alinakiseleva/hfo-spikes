function [results, extracted_features, tfz,tf,f] = Delphos_detector(signal, labels, chan_type, Fs, detection_type, freq_band, artefact, thr_type, param_thr)
% Input:
...
for i = 1:n_chan
    ...
    % Normalized the TF
    [tfz, ~, tf_real_modif, sigma] = z_H0(tf, Fs, artefact_bln); % tfz: time x freq
    ...
    if ~isempty(t_max)
        ...
        FWHMt = ceil(1/pi*sqrt(2*log(2)*VanMom)*Fs./f(f_max)); % time width at half max value
        MAX = [t_max, f_max, value_max, DL, DH, Area, L1, L2, H1, H2, FWHMt];
        
        extracted_features = [extracted_features; MAX];
...
function [DL, DH, Area, L1, L2, H1, H2] = region_half_high_charac( tf_z_thr, t_max, f_max, value_max, Fs )
    ...
    for k = 1:length(t_max)
        ...
        % if closer than 0.5 sec to border,,,,goodbye
        ...
        a = find(tf_z_thr(t_max(k),f_max(k)+1:end) < value_max(k)/2 | tf_z_thr(t_max(k),f_max(k)+1:end) > value_max(k),1,'first'); % f^^^^ power goes down after peak or goes up 
        b = f_max(k) - find(tf_z_thr(t_max(k),1:f_max(k)) < value_max(k)/2 | tf_z_thr(t_max(k),1:f_max(k)) > value_max(k),1,'last'); % f\/\/\/ low power in freq lower than peak
        ...
        if isempty(a) % high freq power
            if isempty(b) % low freq power
    ...
    DL = L1+L2;
    DH = H1+H2;
    Area = pi.*DH.*DL/4; % area around the peak which stays above max_value/2 
...
function [tf_z, tf2, tf_real_modif, sigma_real_N, Q] = z_H0(tf, Fs, artefact_bln)
    ...
    if size(tf,2) > size(tf,1) %  it must be time x freq
        tf = tf';
    end
