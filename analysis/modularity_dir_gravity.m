function [Ci,Q,paramsStruct]=modularity_dir_gravity(A,gamma,spatialLocs,nodeImportance)
...
% Josh Diamond 
% Jan 2019 
% Unaffiliated with toolbox 
% Implementing Expert et. al. PNAS 2011 Uncovering space-independent
...
Ko = sum(A,1);                            %out-degree
Ki = sum(A,2);                            %in-degree
m=sum(Ki);                           	%number of edges


if ~exist('nodeImportance','var')
    % NG model 
    nodeImportance = (Ki * Ko);
end
%% Build null model  
...
allDistances = squareform(pdist(spatialLocs));
[~,sortInds] = sort(allDistances(:));
indsBuffer = buffer(sortInds,200);

numBins = size(indsBuffer,2);
fd = zeros(1,numBins);
fdSquare = zeros(size(A));

for ii = 1:numBins 
    binInds = indsBuffer(indsBuffer(:,ii) ~= 0, ii);
    
    fd(ii) = sum(A(binInds)) / sum(nodeImportance(binInds));
    fdSquare(binInds) = fd(ii);
end

pSpa = nodeImportance .* fdSquare; 
...
paramsStruct.A = A;
paramsStruct.fdSquare = fdSquare; 
paramsStruct.pNG = nodeImportance; 
paramsStruct.spatialLocs = spatialLocs;
paramsStruct.pSpa = pSpa; 
paramsStruct.modMatrix = b;
